/**
 * HONEYCHAIN — Self-Contained Standalone QR Code Engine (SIH 2026)
 * Generates genuine ISO/IEC 18004 QR codes without requiring external CDNs.
 */

(function(global) {
  // Minimalist, robust QR Code Model 2 generator
  ...
  QRCodeModel.createData = function(typeNumber, errorCorrectLevel, dataList) {
    ...
    if (buffer.getLengthInBits() > totalDataCount * 8) {
      throw new Error("Data overflow! Increase QR code typeNumber.");
    }
  ...
  /**
   * Main HoneyChain QR Rendering Function
   */
  global.HoneyChainQR = {
    render: function(container, text, options) {
      options = options || {};
      ...
      // Select typeNumber dynamically based on text length
      let typeNumber = 4;
      if (text.length > 70) typeNumber = 6;
      if (text.length > 130) typeNumber = 8;

      try {
        const qr = new QRCodeModel(typeNumber, 0); // Level M
        qr.addData(text);
        qr.make();
        ...
        console.error("HoneyChain QR Generation error:", err);
        if (container) {
          container.innerHTML = `<div class="error-fallback" style="padding:1rem;">
            <p><strong>QR Generation Failed</strong></p>
            <p style="font-size:0.8rem;">${err.message || "Invalid payload"}</p>
          </div>`;
        }
        return null;
      }
    ...
    download: function(canvas, filename) {
      ...
      try {
        ...
        link.download = filename || "honeychain-qr.png";
        link.href = canvas.toDataURL("image/png");
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      } catch (e) {
        console.error("HoneyChain: QR Download failed", e);
      }
    }
  };
})(window);
